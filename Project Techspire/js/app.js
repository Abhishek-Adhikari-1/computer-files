...
var wedge_text_position = 0.65; // Distance Along Radius
...
function spinWheel() {
	// Desired winner's name
	let winnerName = "Cammy"; // Replace with the name you want to win

	// Get the index of the desired winner in nameList
	let winnerIndex = nameList.indexOf(winnerName);

	if (winnerIndex === -1) {
		console.error("Winner not found in the list.");
		return;
	}

	// Calculate the angle for the winner based on the number of wedges
	let wedgeAngle = 360 / nameList.length;

	// Calculate the rotation angle for the winner's position
	let targetRotation = winnerIndex * wedgeAngle;

	// Spin the wheel multiple times (5 full rotations) and then stop at the winner
	rotationSpeed += MAX_ROTATION_SPEED;

	// Adjust the rotation speed and target the correct wedge angle
	wheelRotation = 360 * 5 - targetRotation; // 5 full spins then stop at the winner
}
...
function copyLink() {
	clipboard.innerText = encodeURI(
		window.location.href + "#" + JSON.stringify(nameList)
	);
	clipboard.select();
...
function loop() {
	...
	for (i = 0; i < nameList.length; i++) {
		...
		context.translate(
			CANVAS_MID_X +
				Math.cos(degRad(wedgeRotation)) *
					(WHEEL_RADIUS * wedge_text_position),
			CANVAS_MID_Y +
				Math.sin(degRad(wedgeRotation)) *
					(WHEEL_RADIUS * wedge_text_position)
		);
